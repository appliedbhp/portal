...
async function initSuperbillSection(root) {
  ...
  try {
    const [settingsRes, profileRes, histRes] = await Promise.all([
      apiCall("getBillingSettings", {}),
      apiCall("getClientBillingProfile", {}),
      apiCall("getSuperbills", {}).catch(() => ({ superbills: [] }))
    ]);
    renderSuperbill(root, settingsRes.settings || {}, profileRes.profile || {}, histRes.superbills || []);
  } catch (e) {
...
function renderSuperbill(root, settings, profile, superbills) {
  ...
  root.innerHTML = `
    ...
    <!-- Superbill History -->
    <div class="card">
      <h2><i class="bi bi-clock-history"></i> Superbill History</h2>
      ${superbillHistoryHtml(superbills)}
    </div>
...
function superbillHistoryHtml(superbills) {
  if (!superbills || !superbills.length) {
    return `<p style="color:var(--muted);font-size:13px;margin:0;">No superbills generated yet.</p>`;
  }
  const fmtD = d => {
    ...
    <div style="overflow-x:auto;">
      <table class="summary-table">
        ...
        <tbody>
          ${superbills.map(s => `
            <tr>
              <td style="font-size:12px;">${escapeHtml(fmtD(s.generatedAt))}</td>
              <td style="font-size:12px;">${escapeHtml(fmtD(s.dateFrom))} – ${escapeHtml(fmtD(s.dateTo))}</td>
              <td style="text-align:center;">${s.sessionCount}</td>
              <td>$${s.totalFee.toFixed(2)}</td>
              <td style="color:${s.balanceDue > 0 ? "#dc2626" : "#059669"};font-weight:700;">$${s.balanceDue.toFixed(2)}</td>
              <td>
                <a href="${escapeHtml(s.docUrl)}" target="_blank" rel="noopener"
...
async function initClientBillingSection(root) {
  root.innerHTML = `<div class="card"><p style="color:var(--muted);font-size:14px;">Loading billing…</p></div>`;
  try {
    const [superbillRes, billingRes] = await Promise.all([
      apiCall("getSuperbills", {}).catch(() => ({ superbills: [] })),
      apiCall("getClientBilling", {}).catch(() => ({ invoices: [], portalUrl: null, amountDue: 0 }))
    ]);
    const superbills = superbillRes.superbills || [];
    const invoices   = billingRes.invoices    || [];
    const portalUrl  = billingRes.portalUrl   || null;
    const amountDue  = billingRes.amountDue   || 0;

    const balanceColor = amountDue > 0 ? "#dc2626" : "#059669";
    const balanceBadge = amountDue > 0
      ? `<div style="display:inline-flex;align-items:center;gap:8px;background:#fee2e2;border:1.5px solid #fca5a5;
                     border-radius:10px;padding:10px 16px;margin-bottom:16px;">
           <i class="bi bi-exclamation-triangle-fill" style="color:#dc2626;font-size:18px;"></i>
           <div>
             <div style="font-weight:700;color:#991b1b;">Balance Due: $${amountDue.toFixed(2)}</div>
             <div style="font-size:12px;color:#dc2626;">Please pay your outstanding invoice below.</div>
           </div>
           ${portalUrl ? `<a href="${escapeHtml(portalUrl)}" target="_blank" rel="noopener"
              style="margin-left:12px;background:#dc2626;color:#fff;border-radius:8px;padding:8px 16px;
                     text-decoration:none;font-weight:700;font-size:13px;white-space:nowrap;">
              Pay Now <i class="bi bi-arrow-right"></i>
            </a>` : ""}
         </div>`
      : `<div style="display:inline-flex;align-items:center;gap:8px;background:#d1fae5;border:1.5px solid #6ee7b7;
                     border-radius:10px;padding:10px 16px;margin-bottom:16px;">
           <i class="bi bi-check-circle-fill" style="color:#059669;font-size:18px;"></i>
           <div style="font-weight:700;color:#065f46;">No balance due — you're all caught up!</div>
         </div>`;

    const invoiceSection = invoices.length ? `
      <div class="card">
        <h2><i class="bi bi-file-earmark-text-fill"></i> My Invoices</h2>
        ${clientInvoiceListHtml(invoices)}
        ${portalUrl ? `
          <div style="margin-top:16px;">
            <a href="${escapeHtml(portalUrl)}" target="_blank" rel="noopener"
               style="display:inline-flex;align-items:center;gap:8px;background:var(--primary);color:#fff;
                      border-radius:8px;padding:10px 18px;text-decoration:none;font-weight:700;font-size:13px;">
              <i class="bi bi-credit-card-2-front-fill"></i> Manage Billing &amp; Payment Methods
            </a>
            <p style="font-size:12px;color:var(--muted);margin:8px 0 0;">
              Update your card, download receipts, and manage your billing info on Stripe's secure portal.
            </p>
          </div>` : ""}
      </div>` : "";
    ...
    root.innerHTML = `
      <div class="card">
        <h1><i class="bi bi-credit-card-2-front-fill"></i> My Billing</h1>
        <p style="color:var(--muted);font-size:14px;margin:0 0 16px;">
          View your invoices, pay balances, and manage your payment info.
        </p>
        ${balanceBadge}
      </div>
      ${invoiceSection}
      <div class="card">
        <h2><i class="bi bi-receipt-cutoff"></i> Superbills</h2>
        <p style="color:var(--muted);font-size:13px;margin:0 0 16px;">
          Superbills are generated by your provider and can be submitted to your insurance for reimbursement.
        </p>
        ${superbillHistoryHtml(superbills)}
      </div>`;
  } catch (e) {
...
function clientInvoiceListHtml(invoices) {
      ...
      ${invoices.map(inv => {
        ...
        return `
          <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;
            ...
            <div>
              <div style="font-weight:700;font-size:13px;">${escapeHtml(desc.slice(0, 70))}</div>
              <div style="font-size:12px;color:var(--muted);margin-top:2px;">
                ${fmtDate(inv.created)}${inv.due_date ? " · Due " + fmtDate(inv.due_date) : ""}
              </div>
            </div>
            <div style="display:flex;align-items:center;gap:10px;">
              <span style="font-weight:700;font-size:15px;">$${amt}</span>
              <span style="font-size:11px;font-weight:700;padding:2px 8px;border-radius:8px;${style};">${inv.status}</span>
              ${inv.hosted_invoice_url && inv.status !== "paid" ? `
                <a href="${escapeHtml(inv.hosted_invoice_url)}" target="_blank" rel="noopener"
                   style="background:var(--primary);color:#fff;border-radius:7px;padding:6px 14px;
