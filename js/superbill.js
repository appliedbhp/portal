...
async function initClientBillingSection(root) {
  root.innerHTML = `<div class="card"><p style="color:var(--muted);font-size:14px;">Loading superbills…</p></div>`;
  try {
    const res = await apiCall("getSuperbills", {});
    const superbills = res.superbills || [];
    root.innerHTML = `
      <div class="card">
        <h1><i class="bi bi-receipt-cutoff"></i> My Superbills</h1>
    ...
    root.innerHTML = `<div class="card"><div class="alert alert-error">
      <i class="bi bi-exclamation-triangle-fill"></i>
      <span>Could not load superbills: ${escapeHtml(e.message)}</span>
    </div></div>`;
  }
}
